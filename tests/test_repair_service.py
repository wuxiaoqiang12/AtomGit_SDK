...
class TestRepairService:
    ...
    def test_reply_to_comment_with_discussion_id(self):
        ...
        assert call_args.kwargs["body"]["in_reply_to"] == 123
        assert call_args.kwargs["body"]["discussion_id"] == "abc456"
        assert "Test reply" in call_args.kwargs["body"]["body"]
    ...
    def test_reply_to_comment_without_discussion_id(self):
        ...
        call_args = self.mock_client.request.call_args
        assert call_args.kwargs["body"]["in_reply_to"] == 124
        assert "discussion_id" not in call_args.kwargs["body"]
