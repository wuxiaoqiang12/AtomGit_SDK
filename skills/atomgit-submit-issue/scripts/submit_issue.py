...
def parse_args():
    ...
    parser.add_argument(
        "--state", choices=["open", "closed"], help="Issue state for update"
    )
    parser.add_argument(
        "--fetch-info", action="store_true", help="Fetch issue info to JSON"
    )
    parser.add_argument("--config", default="config.json", help="Path to config.json")
    parser.add_argument("--output-dir", default="tmp", help="Output directory for JSON")
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview action without executing"
    )
...
def main():
    ...
    try:
        result = service.create_issue(
            title=args.title, body=args.body or "", labels=labels, assignees=assignees
        )
