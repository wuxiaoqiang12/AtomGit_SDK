...
def load_sdk_config(config_path: str) -> AtomGitConfig:
    """加载 SDK 配置"""
    return AtomGitConfig.from_json(config_path)
...
def main():
    ...
    parser.add_argument(
        "--description-file", type=str, help="从文件读取 PR 描述 (Markdown 格式)"
    )
    parser.add_argument(
    ...
    try:
        sdk_config = load_sdk_config(args.config)
    ...
    if args.description_file:
        ...
        print("❌ 错误: 必须提供 PR 描述。")
        print(
            "   建议先让 AI 分析变更并生成专业描述文件，然后使用 --description-file 参数。"
        )
